(function() {
  ...
  this.WOW = (function() {
    ...
    document.addEventListener('DOMContentLoaded', function () {
      const scrollToSection = (buttonSelector, targetSelector) => {
        const button = document.querySelector(buttonSelector);
        const targetSection = document.querySelector(targetSelector);
    
        if (button && targetSection) {
          button.addEventListener('click', function (event) {
            event.preventDefault(); // Zatrzymuje domyślną nawigację
            console.log(`Scrolling to ${targetSelector}`); // Debugowanie
            targetSection.scrollIntoView({
              behavior: 'smooth', // Płynne przewijanie
              block: 'start' // Wyrównanie do górnej części sekcji
            });
          });
        } else {
          console.warn(`Element nie znaleziony: ${buttonSelector} lub ${targetSelector}`);
        }
      };
    
      // Przycisk „Poznaj naszą kolekcję” przewija do „Services”
      scrollToSection('.btn-primary[href="#portfolio"]', '#portfolio');
      scrollToSection('.contact-link[href="#contact"]', '#contact');
